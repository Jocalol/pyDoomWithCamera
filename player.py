...
class Player:
    ...
    def keys_control(self, state, shooting_state):
        ...
        if keys[pygame.K_ESCAPE]:
            exit()
        if (keys[pygame.K_w]) or state == "top":
            dx, dy = player_speed * cos_a, player_speed * sin_a
            self.detect_collision(dx, dy)
        if (keys[pygame.K_s]) or state == "bottom":
            dx, dy = -player_speed * cos_a, -player_speed * sin_a
            self.detect_collision(dx, dy)
        if keys[pygame.K_a] or state == "middle_left":
            dx, dy = player_speed * sin_a, -player_speed * cos_a
            self.detect_collision(dx, dy)
        if keys[pygame.K_d] or state == "middle_right":
            dx, dy = -player_speed * sin_a, player_speed * cos_a
            self.detect_collision(dx, dy)

        if (keys[pygame.K_LEFT]) or state == "top_left" or state == "bottom_left":
